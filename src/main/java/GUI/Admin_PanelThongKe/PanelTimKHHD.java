...
public class PanelTimKHHD extends JPanel {
        ...
        public PanelTimKHHD() {
                ...
                cbTheoCot = new JComboBox<>(
                                new String[] { "Mã đơn hàng", "Ngày TT", "Thành tiền", "Mã nhân viên",
                                                "Mã khuyến mãi" });
        ...
        public ArrayList<DonHangDTO> ketqua(int MaKH) {
                int maDH = txtMaDonHang.getText().trim().isEmpty() ? 0
                                : Integer.parseInt(txtMaDonHang.getText().trim()),
                                maNV = txtMaNhanVien.getText().trim().isEmpty() ? 0
                                                : Integer.parseInt(txtMaNhanVien.getText().trim()),
                                maKM = txtMaKhuyenMai.getText().trim().isEmpty() ? 0
                                                : Integer.parseInt(txtMaKhuyenMai.getText().trim()),
                                maSP = txtMaSanPham.getText().trim().isEmpty() ? 0
                                                : Integer.parseInt(txtMaSanPham.getText().trim()),
    ...
    public ArrayList<String> stringsearch(){
        ...
        if((int) minThanhTien.getValue()!=0 || (int) maxThanhTien.getValue()!=0){
                search.add("Thành tiền từ: "+TienIch.formatVND((int) minThanhTien.getValue())+" đến: "+TienIch.formatVND((int) maxThanhTien.getValue()));
        }
