...
// ==============panel tìm kiếm sắp xếp =============================================
        ...
        //  lbl_discountId = new JLabel("mã đơn hàng");
        // pn_findOrder.add(lbl_discountId, gbc);
        // JPanel pn_discountId = new JPanel(new BorderLayout());
        // pn_discountId.add(lbl_discountId, BorderLayout.NORTH);
        // pn_discountId.add(txt_discountId, BorderLayout.SOUTH);
...
//         add(pn_findOrder, gbc);
    ...
    public static JPanel styledItemInput(JPanel pn, JLabel lbl, JTextField txt) {
        // GridLayout với 2 hàng, 1 cột, khoảng cách giữa dòng là 10px
        pn = new JPanel(new GridLayout(2, 1, 0, 0));
        pn.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10)); // Padding ngoài
    ...
    public static void main(String[] args) {
        // Áp dụng giao diện hiện đại hơn (FlatLaf, nếu cần)
        try {
        ...
        JFrame frame = new JFrame("Form Nhập Liệu");
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        ...
        panel.setBorder(BorderFactory.createEmptyBorder(20, 20, 20, 20)); // Tạo khoảng cách
        panel.setBackground(new Color(240, 248, 255)); // Màu nền nhẹ (xanh pastel)

        GridBagConstraints gbc = new GridBagConstraints();
        gbc.insets = new Insets(8, 5, 8, 5); // Khoảng cách giữa các thành phần
        gbc.fill = GridBagConstraints.HORIZONTAL;
        gbc.anchor = GridBagConstraints.WEST; // Căn lề trái

        JPanel pn = new JPanel();
        JLabel Label = new JLabel("flk");
        JTextField JTextField= new JTextField();
      
        pn = styledItemInput(pn, Label, JTextField);
